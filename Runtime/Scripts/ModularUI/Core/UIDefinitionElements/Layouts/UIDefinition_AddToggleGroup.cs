...
namespace GentlyUI.ModularUI {
    public abstract partial class UIDefinition {
        ...
        protected GMToggleGroup AddToggleGroup(string contentType, bool allowSwitchOff = false) {
            ...
            GMToggleGroup toggleGroup = UISpawner<GMToggleGroup>.SpawnImmediately(path, currentContainer, currentHierarchyOrder);
            toggleGroup.allowSwitchOff = allowSwitchOff;
            CacheUIObject(toggleGroup.gameObject, () => UISpawner<GMToggleGroup>.ReturnImmediately(toggleGroup));
