...
namespace Uween
{
    ...
    public static class EasingHelper {
        ...
        /// <summary>
        /// Returns a simple (parameterless) easing method.
        /// </summary>
        public static void ApplySimpleEasing(EasingType type, Tween tween) {
            switch(type) {
    ...
    public static class CustomEase {
        public static T EaseCustomCurve<T>(this T tween, AnimationCurve animationCurve, bool useDifferenceForCurveEvaluation) where T : Tween {
            if (tween.Easing is EaseCurve easeCurve) {
                easeCurve.curve = animationCurve;
                easeCurve.useDifference = useDifferenceForCurveEvaluation;
            } else {
                tween.Easing = new EaseCurve(animationCurve, useDifferenceForCurveEvaluation);
            }
            return tween;
        }


        public class EaseCurve : Easings {
            public AnimationCurve curve;
            public bool useDifference;

            public EaseCurve(AnimationCurve curve, bool useDifference) {
                this.curve = curve;
                this.useDifference = useDifference;
            }

            public override float Calculate(float t, float startValue, float endValue, float d) {
                float normalizedTime = t / d;
                if (useDifference) {
                    float diff = Mathf.Abs(endValue - startValue);
                    if (startValue > endValue) diff *= -1;
                    return startValue + curve.Evaluate(normalizedTime) * diff;
                } else {
                    return curve.Evaluate(normalizedTime) * endValue;
                }
            }

            public Vector2 Calculate(float t, Vector2 fromValue, Vector2 toValue, float d) {
                Vector2 value = Vector2.one;

                value.x = Calculate(t, fromValue.x, toValue.x, d);
                value.y = Calculate(t, fromValue.y, toValue.y, d);

                return value;
            }

            public Vector3 Calculate(float t, Vector3 fromValue, Vector3 toValue, float d) {
                Vector3 value = Vector3.one;

                value.x = Calculate(t, fromValue.x, toValue.x, d);
                value.y = Calculate(t, fromValue.y, toValue.y, d);
                value.z = Calculate(t, fromValue.z, toValue.z, d);

                return value;
            }

            public Vector4 Calculate(float t, Vector4 fromValue, Vector4 toValue, float d) {
                Vector4 value = Vector4.one;

                value.x = Calculate(t, fromValue.x, toValue.x, d);
                value.y = Calculate(t, fromValue.y, toValue.y, d);
                value.z = Calculate(t, fromValue.z, toValue.z, d);
                value.w = Calculate(t, fromValue.w, toValue.w, d);
